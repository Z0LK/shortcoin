...
const a = ASSETS.find(x => x.symbol === 'TSLAx')!
...
let ticks = 0
eng.subscribe('TSLAx', () => ticks++)
eng.prime('TSLAx', c.at(-1)!)
eng.start(); await new Promise(r => setTimeout(r, 400)); eng.stop()
console.log('engine ticks received:', ticks, '| price moved:', eng.snapshot('TSLAx')!.price.toFixed(2))
