  ...
  return (
    <section className="panel flex flex-col gap-3 p-3">
      <header className="flex items-center justify-between">
        <h2 className="font-display text-sm font-semibold">{t('ticket.title')}</h2>
        <button
      ...
      {showHelp && (
        <ul className="flex flex-col gap-1.5 rounded-[12px] border border-line bg-sunken p-2.5">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex h-11 items-center rounded-[12px] border border-line bg-sunken px-3 focus-within:border-line-strong">
        ...
        <div className="grid grid-cols-4 gap-1">
          {PRESETS.map((f) => (
            <button
              ...
              className="h-8 rounded-[12px] border border-line bg-raised text-mini font-semibold text-ink-2 hover:text-ink"
            >
      ...
      {q ? (
        ...
        <p className="rounded-[12px] border border-line bg-sunken p-2.5 text-mini text-ink-3">
      ...
      {/* ── rates and cost ─────────────────────────────────────────────── */}
      {q && (
        <div className="flex flex-col gap-2 rounded-[12px] border border-line bg-sunken p-2.5">
          <div className="grid grid-cols-2 gap-2" title={t('ticket.rateHint')}>
            <div className={cn('rounded-[12px] p-1 transition-colors duration-700', changed.has('dailyRateBps') && 'bg-warn/15')}>
            ...
            <div className={cn('rounded-[12px] p-1 transition-colors duration-700', changed.has('marginalRateBps') && 'bg-warn/15')}>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-1 text-center">
              {([1, 7, 30] as const).map((d) => (
                <div key={d} className="rounded-[12px] border border-line bg-surface py-1">
          ...
          <div className={cn('rounded-[12px] transition-colors duration-700', changed.has('utilizationAfter') && 'bg-warn/15')}>
          ...
          <div className={cn('rounded-[12px] transition-colors duration-700', changed.has('backing') && 'bg-warn/15')} title={t('ticket.backingHint')}>
            <p className="mb-1 text-micro text-ink-4">{t('ticket.backing')}</p>
            <div className="flex h-2 w-full overflow-hidden rounded-[12px]">
      ...
      {q && (
        <div className="grid grid-cols-2 gap-2 text-micro">
          <div className="rounded-[12px] border border-line p-1.5" title={t('ticket.payoutEligibleHint')}>
      ...
      {q && startingMark !== null && startingMark < -0.01 && (
        <p className="rounded-[12px] border border-warn/30 bg-warn/10 p-2 text-mini leading-relaxed text-warn">
      ...
      {notice && <p className="rounded-[12px] border border-warn/30 bg-warn/10 p-2 text-mini text-warn">{notice}</p>}
      {sendError && <p className="rounded-[12px] border border-short/30 bg-short/10 p-2 text-mini text-short">{sendError}</p>}
      ...
      {needsApproval && !blocked ? (
        <button
          onClick={approve}
          disabled={phase !== 'idle'}
          className="h-11 w-full rounded-[12px] border border-line-strong bg-raised text-sm font-semibold text-ink transition-colors hover:bg-ink/15 disabled:opacity-50"
        >
      ...
      <button
        ...
        className="h-12 w-full btn-primary text-sm"
      >
